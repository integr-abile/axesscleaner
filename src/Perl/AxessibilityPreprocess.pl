##########################################################
#
# AxessibilityPreprocess v1.0
#
# Based on the script made available by cmhughes in the reply at 
...
while(<MAINFILE>)
{
   ...
   #if line is a comment, push immediately in @lines and loop
   if(substr($_, 0, 1) ne  "%")
   {
   
   #if line contains a comment, save it in $commentpart and leave in $_  the part of line until before first %    
   if(index($_, "%") > 0)
   {
   	$commentpart = substr($_, index($_, "%"));
   ...
   else
   {
    ...
    #reattach comment part at the end of the modified line (if needed)
    if ($commentpart ne "")
    {
    	$_ .= $commentpart;
    }
